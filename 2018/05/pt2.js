...
if (allLetters.length !== 26) throw 'whoops';

const value = allLetters.split('').reduce((result, char) => {
  return Math.min(result, reducePolymer(
    fileContents.split('')
      .filter(contentsChar => contentsChar.toLowerCase() !== char.toLowerCase())
      .join('')
  ).length);
}, Infinity);
